...
use sidan_csl_rs::{
    csl::JsError,
    model::{
        AccountInfo, AssetMetadata, BlockInfo, GovernanceProposalInfo, Protocol, TransactionInfo,
        UTxO,
    },
};
...
pub trait Fetcher: Send + Sync {
    ...
    async fn fetch_asset_metadata(&self, asset: &str) -> Result<AssetMetadata, Box<dyn Error>>;
    async fn fetch_block_info(&self, hash: &str) -> Result<BlockInfo, Box<dyn Error>>;
    async fn fetch_collection_assets(
        &self,
        policy_id: &str,
        cursor: Option<String>,
    ) -> Result<(Vec<(String, String)>, Option<(u64, String)>), Box<dyn Error>>;
