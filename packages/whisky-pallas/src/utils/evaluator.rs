use crate::converter::bytes_from_bech32;
use crate::tx_parser;
...
use pallas_primitives::conway::Redeemer;
use pallas_primitives::conway::{
    AssetName, Coin, DatumOption, PlutusData, PolicyId, PostAlonzoTransactionOutput,
    RedeemerTag as PallasRedeemerTag, ScriptRef, TransactionOutput, Value,
};
use std::collections::BTreeMap;
use std::collections::HashMap;
use std::str::FromStr;
use uplc::tx::SlotConfig;
use uplc::Fragment as UplcFragment;
...
pub fn evaluate_tx_scripts(
    ...
    let tx = Tx::decode_fragment(&tx_bytes)
        .map_err(WError::from_err("evaluate_tx_scripts - tx decode"))?;

    let mut all_utxos = inputs.to_vec();
    for additional_tx in additional_txs {
        let additional_utxos = tx_parser::extract_tx_utxos(additional_tx)
            .map_err(WError::from_err("evaluate_tx_scripts - extract_tx_utxos"))?;
...
pub fn map_eval_result(result: PhaseTwoEvalResult) -> EvalResult {
    match result {
        PhaseTwoEvalResult::Success(redeemer) => EvalResult::Success(Action {
...
pub fn map_redeemer_tag(tag: &PallasRedeemerTag) -> RedeemerTag {
    match tag {
        PallasRedeemerTag::Spend => RedeemerTag::Spend,
        PallasRedeemerTag::Mint => RedeemerTag::Mint,
        PallasRedeemerTag::Cert => RedeemerTag::Cert,
        PallasRedeemerTag::Reward => RedeemerTag::Reward,
        PallasRedeemerTag::Vote => RedeemerTag::Vote,
        PallasRedeemerTag::Propose => RedeemerTag::Propose,
    }
}
    ...
    for utxo in utxos {
        ...
        let resolved_input = ResolvedInput {
            ...
            output: TransactionOutput::PostAlonzo(PostAlonzoTransactionOutput {
                address: Bytes::from_str(&bytes_from_bech32(&utxo.output.address)?).map_err(
                    |err| {
                        WError::new(
                            "to_pallas_utxos",
