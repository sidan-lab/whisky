...
impl CSLParser {
    ...
    pub(super) fn extract_change_address(&mut self) -> Result<(), WError> {
        ...
        if outputs_len > 0 {
            let change_address = outputs.get(outputs_len - 1).address().to_bech32(None).map_err(|e| {
                WError::new("extract_change_address", &format!("Failed to convert change address to bech32: {}", e))
            })?;
