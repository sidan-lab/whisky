...
pub fn evaluate_tx_scripts(
    ...
    let tx = match mtx {
        ...
        _ => return Err(WError::from_str("Invalid Tx Era")),
    };
    ...
    .map_err(|err| WError::from_str(&format!("Error occurred during evaluation: {}", err)))
...
fn get_cost_mdls(network: &Network) -> Result<CostModels, WError> {
...
fn to_pallas_utxos(utxos: &Vec<UTxO>) -> Result<Vec<ResolvedInput>, WError> {
    let mut resolved_inputs = Vec::new();
    for utxo in utxos {
        let tx_hash: [u8; 32] = hex::decode(&utxo.input.tx_hash)
            .map_err(|err| WError::from_str(&format!("Invalid tx hash found: {}", err)))?
            .try_into()
            .map_err(|_e| WError::from_str("Invalid tx hash length found"))?;
        ...
        let resolved_input = ResolvedInput {
            ...
            output: TransactionOutput::PostAlonzo(PostAlonzoTransactionOutput {
                address: Bytes::from(csl::Address::from_bech32(&utxo.output.address)?.to_bytes()),
...
fn to_pallas_script_ref(
    ...
    if let Some(script_ref) = script_ref {
        let script_bytes = hex::decode(script_ref)
            .map_err(|err| WError::from_str(&format!("Invalid script hex found: {}", err)))?;

        let pallas_script = ScriptRef::decode_fragment(&script_bytes)
            .map_err(|err| WError::from_str(&format!("Invalid script found: {}", err)))?;
...
fn to_pallas_datum(utxo_output: &UtxoOutput) -> Result<Option<DatumOption>, WError> {
    if let Some(inline_datum) = &utxo_output.plutus_data {
        //hex to bytes
        let plutus_data_bytes = hex::decode(inline_datum)
            .map_err(|err| WError::from_str(&format!("Invalid plutus data found: {}", err)))?;
        let datum = CborWrap(
            PlutusData::decode_fragment(&plutus_data_bytes)
                .map_err(|_e| WError::from_str("Invalid plutus data found"))?,
        );
        ...
        let datum_hash_bytes: [u8; 32] = hex::decode(datum_hash)
            .map_err(|err| WError::from_str(&format!("Invalid datum hash found: {}", err)))?
            .try_into()
            .map_err(|_e| WError::from_str("Invalid byte length of datum hash found"))?;
...
fn to_pallas_value(assets: &Vec<Asset>) -> Result<Value, WError> {
    if assets.len() == 1 {
        match assets[0].unit().as_str() {
            "lovelace" => Ok(Value::Coin(assets[0].quantity().parse::<u64>().unwrap())),
            _ => Err(WError::from_str("Invalid value")),
        }
    } else {
...
fn to_pallas_multi_asset_value(assets: &Vec<Asset>) -> Result<Value, WError> {
    ...
    for (policy_id, asset_list) in &asset_mapping {
        let policy_id_bytes: [u8; 28] = hex::decode(policy_id)
            .map_err(|err| WError::from_str(&format!("Invalid policy id found: {}", err)))?
            .try_into()
            .map_err(|_e| WError::from_str("Invalid length policy id found"))?;
