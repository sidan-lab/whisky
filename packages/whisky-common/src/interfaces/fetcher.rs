...
pub trait Fetcher: Send + Sync {
    ...
    async fn fetch_asset_metadata(&self, asset: &str) -> Result<AssetMetadata, WError>;
    async fn fetch_block_info(&self, hash: &str) -> Result<BlockInfo, WError>;
    async fn fetch_collection_assets(
        &self,
        policy_id: &str,
        cursor: Option<String>,
    ) -> Result<(Vec<(String, String)>, Option<(u64, String)>), WError>;
    async fn fetch_protocol_parameters(&self, epoch: u32) -> Result<Protocol, WError>;
    async fn fetch_tx_info(&self, hash: &str) -> Result<TransactionInfo, WError>;
    async fn fetch_utxos(&self, hash: &str, index: Option<usize>) -> Result<Vec<UTxO>, WError>;
    async fn fetch_governance_proposal(
        &self,
        tx_hash: &str,
        cert_index: usize,
    ) -> Result<GovernanceProposalInfo, WError>;
