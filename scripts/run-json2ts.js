...
const schemasDir = path.join('packages', 'whisky-js', 'json-gen', 'schemas');
...
Promise.all(schemaFiles.map(schemaFile => {
  ...
  */
  ...
  fs.mkdirSync(path.join('packages', 'whisky-js', 'json-gen', 'output'), { recursive: true });
  ...
  return fs.writeFileSync(path.join('packages', 'whisky-js', 'json-gen', 'output', 'json-types.d.ts'), dedupedDefs.join('\n'));
});
