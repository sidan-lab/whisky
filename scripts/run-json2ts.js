...
const schemasDir = path.join('packages', 'sidan-csl-rs', 'json-gen', 'schemas');
...
Promise.all(schemaFiles.map(schemaFile => {
  ...
  */
  ...
  fs.mkdirSync(path.join('packages', 'sidan-csl-rs', 'json-gen', 'output'), { recursive: true });
  ...
  return fs.writeFileSync(path.join('packages', 'sidan-csl-rs', 'json-gen', 'output', 'json-types.d.ts'), dedupedDefs.join('\n'));
});
